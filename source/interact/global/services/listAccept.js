...
export const listAccept = asyncErrorHandler(async (interaction) => {
    ...
    const list = await prisma.cfs_accept.findMany({
        where: {
            guild: guildId,
        },
    });
    if (list === null || list.length === 0) {
        return {
            flag: true,
            title: "List Accepted Confessions",
            description: `No accepted confessions in the server right now.`,
            color: 0xffb27e
        };
    } else {
        if (list.length > 5) {
            ...
            let array = [];
            const listAccept = await prisma.cfs_accept.findMany({
                where: {
                    guild: guildId,
                },
                take: confessions,
            ...
            for (const confession of listAccept) {
            ...
            return {
                flag: true,
                embed: embedListCFS(guild, array, `ACCEPTED`, list.length, 0xffb27e),
                buttons: [
                    buttonsPagination(1, totalPages, `accept_${guildId}`),
                    buttonsDetail(list, `accept`, guildId)
                ]
            };
        } else {
            let array = [];
